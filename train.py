...
def Train(directory, epochs, aggregator, embedding_size, layers, dropout, slope, lr, wd, random_seed, ctx):
    ...
    print("train_index*train_index*train_index*train_index*train_index")
    print(train_index)
    print(test_index)
    ...
    for i in range(len(train_index)):
        ...
        train_eid = g.filter_edges(lambda edges: edges.data['train']).astype('int64')
        print("0000000000000000000000sssssssssssssssssssssssssssssssssss")
        print(test_eid.shape)
        print(test_eid)
        print(train_eid)
        whole_eid = np.hstack((test_eid,train_eid))
        print(whole_eid.shape)

        # whole_eid = g.filter_edges().astype('int64')
        # print(whole_eid)
        # whole_src,whole_test = g.find_edges(whole_eid)
        ...
        # print(g_train.device)
        # g_train = g_train.device()
    ...
    print(model)
    # dgl.save_graphs('graphcomplete.dgl', model)
