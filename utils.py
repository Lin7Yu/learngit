...
import pandas as pd
import mxnet as mx
from mxnet import gpu, ndarray as nd
import dgl
...
def sample_new(directory, random_seed):
    all_associations = pd.read_csv(directory + '/all_mirna_disease_pairs.csv', names=['miRNA', 'disease', 'label'])
    known_associations = all_associations.loc[all_associations['label'] == 1]
    unknown_associations = all_associations.loc[all_associations['label'] == 0]
    sample_df = known_associations.append(unknown_associations)
    sample_df.reset_index(drop=True, inplace=True)
...
def build_graph(directory, random_seed, ctx):
    ...
    # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")#将模型放到qpu上
